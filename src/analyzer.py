    ...
    "dashboard": {
        ...
        "battle_plan": {
            ...
            "action_checklist": [
                ...
                "✅/⚠️/❌ 检查项6：PE估值合理",
                "✅/⚠️/❌ 检查项7：是否属于当前核心热门题材",
                "✅/⚠️/❌ 检查项8：股性是否活跃（近期有涨停或连板基因）",
                "✅/⚠️/❌ 检查项9：量能与换手率是否充足且健康（拒绝缩量秒板或死亡换手）",
                "✅/⚠️/❌ 检查项10：K线形态是否为强突破（反包/跳空/突破前高）",
                "✅/⚠️/❌ 检查项11：盈亏比是否合理（上方套牢盘压力是否较小）"
            ]
        }
    },

    "analysis_summary": "150字综合分析摘要",
    "key_points": "3-5个核心看点，逗号分隔",
    "risk_warning": "风险提示",
    "buy_reason": "操作理由，引用交易理念",

    "trend_analysis": "走势形态分析",
    "short_term_outlook": "短期1-2日展望",
...
- ✅ 多头排列：MA5 > MA10 > MA20
- ✅ 热门主线：处于当前核心炒作题材内
- ✅ 量价共振：放量突破或缩量洗盘后首阳，换手率充足（10%-30%）
- ✅ 股性活跃：近期有涨停板基因，或走出连板形态
- ✅ 筹码结构好：上方无明显套牢盘阻力
- ✅ 消息面有利好催化或题材发酵

### 买入（60-79分）：
- ✅ 多头排列或弱势多头（有效站稳日内均线）
- ✅ 乖离率 <5%（对于绝对核心连板龙头可适当放宽）
- ✅ 量能正常，未出现极端爆量阴线
- ⚪ 允许一项次要条件不满足（如非最强主线，但板块效应尚可）
...
### 强烈买入（80-100分）：
- ✅ 题材主线：属于当前市场绝对主线/热点核心题材
- ✅ 量价承接：盘口承接极强，爆量分歧转一致或弱转强突破
- ✅ 盈亏比高：上方阻力极小，连板接力/打板盈亏比极高
- ✅ 信号确定：关键数据和情报结论一致，日内上车信号极其确定

### 买入（60-79分）：
- ✅ 热门跟风或首阴：热门题材强势跟风，或前排核心首阴回踩
- ✅ 承接良好：盘口/分时承接尚可，未出现恶劣抛压
- ✅ 趋势未坏：允许日内存在一定分歧，但整体趋势未坏
- ✅ 次优入场：适合半路/低吸等次优入场点，严格控制仓位

### 观望（40-59分）：
- ⚠️ 题材尾声：题材炒作接近尾声或进入混沌期，资金接力意愿不足
- ⚠️ 分歧过大：盘中出现过冲高回落/炸板（大面风险）
- ⚠️ 量价背离：换手率不足（缩量秒板次日风险大）或放量滞涨

### 卖出/减仓（0-39分）：
- ❌ 情绪退潮：核心龙头断板大跌/核按钮，情绪明显退潮
- ❌ 破位无承接：跌破日内均价线/短线生命线且无承接，或出现高位A杀
- ❌ 无条件止损：触发了短线无条件止损纪律或突发重大利空
...
### 重点关注（必须明确回答）：
1. ❓ 题材定位：是否属于当前市场绝对主线或核心热点？（非主线跟风股坚决规避）
2. ❓ 量价结构：今日换手率是否充足？（警惕缩量秒板、缩量一字后的次日抛压）
3. ❓ 盘口承接：日内分时图是否有效站稳均价线，是否出现过炸板或大幅冲高回落大面？
4. ❓ 股性活跃度：近期是否有涨停板基因，或是否走出了明确的连板/反包形态？
5. ❓ 情绪周期匹配：当前大盘情绪是处于主升、退潮还是混沌期？本股属于什么身位（龙头/前排/杂毛）？
6. ❓ 盈亏比评估：上方是否有密集的历史套牢盘阻力？当前接力/打板的盈亏比是否划算？
7. ❓ 异动与龙虎榜：近期是否有明显的游资/机构资金抢筹迹象？
8. ❓ 排雷风控：是否存在减持、立案调查、退市风险等一票否决级别的重大利空？
"""
        else:
            prompt += f"""

### 重点关注（必须明确回答）：
1. ❓ 触发条件：当前资金博弈结构是否完美满足激活技能的关键条件（如弱转强、首阴、龙回头）？
2. ❓ 题材与情绪共振：当前炒作题材是否具有持续性？大盘情绪周期是否支持接力操作？
3. ❓ 盈亏比与套牢盘：当前上车位置是否属于高位接盘？上方阻力区在哪里？
4. ❓ 量能与换手：量能释放是否健康（放量分歧转一致/缩量洗盘），是否出现死亡换手或爆量滞涨？
5. ❓ 分时承接力：盘中遇到抛压时的承接力度如何？是否出现过恶劣的A杀或断板核按钮？
6. ❓ 上车点设定：若看多，具体的上车方式是什么（水下低吸/半路/打板）？对应价格大概在什么区间？
7. ❓ 纪律与止损：明确无条件止损位（如跌破日内均线、跌破关键支撑）和预期持仓周期。
8. ❓ 风险排查：有无与当前炒作逻辑相悖的消息？是否有隐藏的监管、解禁或退市风险？
"""
        prompt += f"""
